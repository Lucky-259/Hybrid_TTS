...
def critic_mcts(
    ...
    task = Task(task_name=config.task_name,save_dir=config.save_dir)
    env = task.env_fn(
        config={
            "max_actions": config.tree_max_width,
            "max_length": config.tree_max_depth,
            ...
            "num_refine": gen_config.num_refine,
            "prm_threshold": gen_config.prm_threshold,
            "refine_cut_num":gen_config.refine_cut_num,
            "prm_gap":gen_config.prm_gap,
            "rewrite_from":gen_config.rewrite_from,
        },
        math_problems=[
            {
                "question": problem_inst["question"],
    ...
    return TreeSearchSolutionOutput(
        solutions=[t["text"] for t in traj_list],
        completion_tokens=[t["api_completion_tokens"] for t in traj_list],
        path_tokens=[t["path_tokens"] for t in traj_list],
        tree_completion_tokens=[t["tree_completion_tokens"] for t in traj_list],
        metadata={
            ...
            "step_PRM": [[f"Path {i}'s step_PRM"] + t['stepprm_path'] for i, t in enumerate(traj_list)],
            "refine_info_list": [t["refine_info_list"] for i, t in enumerate(traj_list)]
        }
    )